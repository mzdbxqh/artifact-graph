...
async function scanArtifacts(root, schema) {
  ...
  for (const [type, definition] of artifactTypeEntriesBySpecificity(config)) {
    ...
    for (const file of files) {
      ...
      const raw = await (0, import_promises5.readFile)((0, import_node_path6.join)(root, file), "utf-8");
...
async function validateScenarioPrdLinkIndex(root, graph) {
  ...
  let raw = "";
  try {
    raw = await (0, import_promises5.readFile)((0, import_node_path6.join)(root, indexPath), "utf-8");
  } catch (error) {
    if (error.code === "ENOENT") {
      return [];
    }
...
async function writeGraphCache(root, graph) {
  const cacheDir = (0, import_node_path6.join)(root, ".artifact-graph");
  await (0, import_promises5.mkdir)(cacheDir, { recursive: true });
  await (0, import_promises5.writeFile)((0, import_node_path6.join)(cacheDir, "index.json"), `${JSON.stringify(graph, null, 2)}
`);
  const db = new import_better_sqlite3.default((0, import_node_path6.join)(cacheDir, "graph.sqlite"));
  try {
    db.exec(`
...
function isTestFile(filePath) {
  ...
  const name = (0, import_node_path6.basename)(normalized);
...
function parseE2eTest(path, raw) {
  ...
  const nodes = [];
  const edges = [];
  const batch = String(data.test_batch ?? (0, import_node_path6.basename)(path, (0, import_node_path6.extname)(path))).trim();
...
function parseE2eRegistry(path, raw) {
  ...
  return {
    nodes: [{ type: "e2e_registry", code: (0, import_node_path6.basename)(path, (0, import_node_path6.extname)(path)), title: "E2E Test Registry", path, line: 1, attrs: data }],
    edges: []
  };
}
...
async function validateExecutableTraceability(root) {
  ...
  for (const filePath of e2eFiles) {
    ...
    const relPath = (0, import_node_path6.relative)(root, filePath).split("\\").join("/");
    const parsed = (0, import_gray_matter.default)(raw);
    const data = parsed.data;
    const batch = String(data.test_batch ?? (0, import_node_path6.basename)(filePath, (0, import_node_path6.extname)(filePath))).trim();
    const lines = raw.split(/\r?\n/);
    const tcStarts = [];
    lines.forEach((line, index) => {
  ...
  for (const [tcKey, { ref, chainType, path, line }] of mdToRef) {
    ...
    for (const entry of refEntries) {
      ...
      if (entry.testId) {
        let content;
        try {
          content = await (0, import_promises5.readFile)((0, import_node_path6.join)(root, normalizedRefFile), "utf-8");
        } catch {
          continue;
        }
...
async function hasMarkdownTc(tcKey, e2eDir) {
  ...
  const filePath = (0, import_node_path6.join)(e2eDir, `${batch}.md`);
  try {
    const raw = await (0, import_promises5.readFile)(filePath, "utf-8");
...
async function walk(root, current = root) {
  ...
  for (const entry of entries) {
    ...
    const fullPath = (0, import_node_path6.join)(current, entry.name);
    if (entry.isDirectory()) {
      files.push(...await walk(root, fullPath));
    } else {
      files.push((0, import_node_path6.relative)(root, fullPath).split("\\").join("/"));
    }
  }
  return files;
}
...
function normalizeDesignCode(value) {
  ...
  return (0, import_node_path6.basename)(raw, (0, import_node_path6.extname)(raw));
}
